  ...
  <div key={row} className="flex gap-2 mt-2">
    <div className="flex flex-wrap items-center justify-center gap-2">
      {Array.from({length: count}, (_, i)=> {
        ...
        return(
          <button key={seatId} onClick={()=> handleSeatClick (seatId) }className={`h-8 w-8 rounded border border-primary/60 cursor-pointer ${selectedSeats.includes(seatId)&& "bg-primary text-white"}`}> {seatId}
          </button>
        );
      })}
    </div>
  ...
  return show ? (
   ...
   <div className='relative flex-1 flex flex-col items-center max-md:mt-16'>
    ...
    <p className='text-gray-400  text-sm mb-6'>SCREEN SIDE</p>
     ...
     <div className ='flex flex-col items-center mt-10 text-xs text-gray-300'></div>
